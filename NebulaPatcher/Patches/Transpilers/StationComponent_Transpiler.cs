            ...
            .Repeat(matcher =>
            {
                matcher
                    ...
                    .InsertAndAdvance(HarmonyLib.Transpilers.EmitDelegate<RemOrderFunc2>((StationComponent stationComponent, int index) =>
                    {
                        ...
                        {
                            List<NetworkConnection> subscribers = StationUIManager.GetSubscribers(stationComponent.planetId, stationComponent.id, stationComponent.gid);
                ...
                matcher
                    ...
                    .InsertAndAdvance(HarmonyLib.Transpilers.EmitDelegate<RemOrderFunc3>((StationComponent stationComponent, StationComponent[] gStationComponent, int n) =>
                    {
                        if (SimulatedWorld.Initialized && LocalPlayer.IsMasterClient)
                        {
                            ...
                            if (storeArray != null)
                            {
                                List<NetworkConnection> subscribers = StationUIManager.GetSubscribers(gStationComponent[gIndex].planetId, gStationComponent[gIndex].id, gStationComponent[gIndex].gid);
