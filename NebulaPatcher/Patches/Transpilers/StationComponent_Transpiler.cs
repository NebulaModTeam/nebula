        ...
        public static IEnumerable<CodeInstruction> InternalTickRemote_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
        {
            ...
            instructions = new CodeMatcher(instructions)
                ...
                .InsertAndAdvance(HarmonyLib.Transpilers.EmitDelegate<ShipFunc>((StationComponent stationComponent, ref ShipData shipData) =>
                {
                    if (SimulatedWorld.Instance.Initialized && LocalPlayer.Instance.IsMasterClient)
                    {
                        ...
                        LocalPlayer.Instance.SendPacketToStar(packet, GameMain.galaxy.PlanetById(stationComponent.planetId).star.id);
                    }
                    return 0;
                }))
                .Insert(new CodeInstruction(OpCodes.Pop))
                ...
                .Repeat(matcher =>
                {
                    if (TakeItemCounter == 0)
                    {
                        ...
                        .InsertAndAdvance(HarmonyLib.Transpilers.EmitDelegate<EnergyCost>((StationComponent stationComponent, long cost) =>
                        {
                            if (SimulatedWorld.Instance.Initialized && LocalPlayer.Instance.IsMasterClient)
                            {
                                LocalPlayer.Instance.SendPacketToStar(new ILSEnergyConsumeNotification(stationComponent.gid, cost), GameMain.galaxy.PlanetById(stationComponent.planetId).star.id);
                            }
                            return 0;
                        }))
                        .InsertAndAdvance(new CodeInstruction(OpCodes.Pop));
            ...
            instructions = new CodeMatcher(instructions)
                ...
                .InsertAndAdvance(HarmonyLib.Transpilers.EmitDelegate<ShipFunc>((StationComponent stationComponent, ref ShipData shipData) =>
                {
                    if (SimulatedWorld.Instance.Initialized && LocalPlayer.Instance.IsMasterClient)
                    {
                        LocalPlayer.Instance.SendPacket(new ILSShipUpdateWarperCnt(stationComponent.gid, shipData.shipIndex, shipData.warperCnt));
                    }
                    return 0;
                }))
                .Insert(new CodeInstruction(OpCodes.Pop))
            ...
            instructions = new CodeMatcher(instructions)
                ...
                .InsertAndAdvance(HarmonyLib.Transpilers.EmitDelegate<Func<StationComponent, int>>(stationComponent =>
                {
                    if (SimulatedWorld.Instance.Initialized && LocalPlayer.Instance.IsMasterClient)
                    {
                        LocalPlayer.Instance.SendPacket(new StationUI(stationComponent.planetId, stationComponent.id, stationComponent.gid, StationUI.EUISettings.SetWarperCount, stationComponent.warperCount, true));
                    }
                    return 0;
                }))
                .InsertAndAdvance(new CodeInstruction(OpCodes.Pop))
            ...
            instructions = new CodeMatcher(instructions)
                ...
                .InsertAndAdvance(HarmonyLib.Transpilers.EmitDelegate<Func<StationComponent, int>>(stationComponent =>
                {
                    if (SimulatedWorld.Instance.Initialized && LocalPlayer.Instance.IsMasterClient)
                    {
                        LocalPlayer.Instance.SendPacket(new StationUI(stationComponent.planetId, stationComponent.id, stationComponent.gid, StationUI.EUISettings.SetWarperCount, stationComponent.warperCount));
                    }
                    return 0;
                }))
                .InsertAndAdvance(new CodeInstruction(OpCodes.Pop))
