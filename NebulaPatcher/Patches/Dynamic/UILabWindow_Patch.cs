    ...
    class UILabWindow_Patch
    {
        ...
        public static void OnItemButtonClick_Prefix(UILabWindow __instance, int index)
        {
            ...
            if (labComponent.researchMode)
            {
                if (GameMain.mainPlayer.inhandItemId > 0 && GameMain.mainPlayer.inhandItemCount > 0)
                {
                    ...
                    if (GameMain.mainPlayer.inhandItemId == id)
                    {
                        ...
                        if (num3 > 0)
                        {
                            LocalPlayer.Instance.SendPacketToLocalStar(new LaboratoryUpdateCubesPacket(labComponent.matrixServed[index] + num3 * 3600, index, __instance.labId, GameMain.localPlanet?.id ?? -1));
                        }
                    }
                }
                else
                {
                    ...
                    {
                        LocalPlayer.Instance.SendPacketToLocalStar(new LaboratoryUpdateCubesPacket(0, index, __instance.labId, GameMain.localPlanet?.id ?? -1));
                    }
                }
            }
            else if (labComponent.matrixMode)
            {
                if (GameMain.mainPlayer.inhandItemId > 0 && GameMain.mainPlayer.inhandItemCount > 0)
                {
                    ...
                    if (num9 > 0)
                    {
                        LocalPlayer.Instance.SendPacketToLocalStar(new LaboratoryUpdateStoragePacket(labComponent.served[index] + num9, index, __instance.labId, GameMain.localPlanet?.id ?? -1));
                    }
                }
                else
                {
                    ...
                    {
                        LocalPlayer.Instance.SendPacketToLocalStar(new LaboratoryUpdateStoragePacket(0, index, __instance.labId, GameMain.localPlanet?.id ?? -1));
                    }
                }
            }
            else
            {
                ...
                LocalPlayer.Instance.SendPacketToLocalStar(new LaboratoryUpdateEventPacket(index, __instance.labId, GameMain.localPlanet?.id ?? -1));
            }
        }
        ...
        public static void OnItemButtonClick_Prefix(UILabWindow __instance)
        {
            ...
            if (labComponent.matrixMode)
            {
                ...
                LocalPlayer.Instance.SendPacketToLocalStar(new LaboratoryUpdateEventPacket(-3, __instance.labId, GameMain.localPlanet?.id ?? -1));
            }
            else if (!labComponent.researchMode)
            {
                //Notify about selection of research mode
                LocalPlayer.Instance.SendPacketToLocalStar(new LaboratoryUpdateEventPacket(-1, __instance.labId, GameMain.localPlanet?.id ?? -1));
            }
        ...
        public static void OnBackButtonClick_Prefix(UILabWindow __instance)
        {
            ...
            if (SimulatedWorld.Initialized)
            {
                LocalPlayer.Instance.SendPacketToLocalStar(new LaboratoryUpdateEventPacket(-2, __instance.labId, GameMain.localPlanet?.id ?? -1));
            }
        }
    }
}
