...
namespace NebulaPatcher.Patches.Dynamic
{
    ...
    class UILabWindow_Patch
    {
        ...
        public static void OnItemButtonClick_Prefix(UILabWindow __instance, int index)
        {
            ...
            if (labComponent.researchMode)
            {
                if (GameMain.mainPlayer.inhandItemId > 0 && GameMain.mainPlayer.inhandItemCount > 0) {
                    ...
                    if (GameMain.mainPlayer.inhandItemId == id)
                    {
                        ...
                        if (num3 > 0)
                        {
                            LocalPlayer.SendPacketToLocalPlanet(new LaboratoryUpdateCubesPacket(labComponent.matrixServed[index] + num3 * 3600, index, __instance.labId));
                        }
                    }
                }
                else
            ...
            else if (labComponent.matrixMode)
            {
                if (GameMain.mainPlayer.inhandItemId > 0 && GameMain.mainPlayer.inhandItemCount > 0)
                {
                    ...
                    if (num9 > 0)
                    {
                        LocalPlayer.SendPacketToLocalPlanet(new LaboratoryUpdateStoragePacket(labComponent.served[index] + num9, index, __instance.labId));
                    }
                }
                else
                {
                    ...
                    {
                        LocalPlayer.SendPacketToLocalPlanet(new LaboratoryUpdateStoragePacket(0, index, __instance.labId));
                    }
                }
            }
            else
            {
                ...
                LocalPlayer.SendPacketToLocalPlanet(new LaboratoryUpdateEventPacket(index, __instance.labId));
            }
        }
        ...
        public static void OnBackButtonClick_Prefix(UILabWindow __instance)
        {
            ...
            LocalPlayer.SendPacketToLocalPlanet(new LaboratoryUpdateEventPacket(-2, __instance.labId));
        }
    }
}
