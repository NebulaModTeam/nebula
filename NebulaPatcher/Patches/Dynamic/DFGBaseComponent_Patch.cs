...
internal class DFGBaseComponent_Patch
{
    ...
    [HarmonyPrefix]
    [HarmonyPatch(nameof(DFGBaseComponent.UnderAttack), new Type[] { typeof(Vector3), typeof(float), typeof(int) })]
    public static bool UnderAttack_Prefix3(DFGBaseComponent __instance, Vector3 center, float radius, int setStateTick)
    {
        if (!Multiplayer.IsActive || __instance.activeTick > 0) return true;
        if (Multiplayer.Session.IsClient) return false;
